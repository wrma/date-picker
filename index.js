...
function renderHtml(year,month) {
    //默认值为当前年月
    if ( !year || !month){
    ...
    templateBody = '<div class="date-picker_body">'+
        ...
        '                </thead>'+
        '                <tbody>'+
                            getDaysHtml(currentYear,currentMonth)+
        '                </tbody>'+
        '            </table>'+
        '        </div>';
    var template = templateHead + templateBody;
    document.querySelector("#wrap").innerHTML = template;
}
function updateDayHtml(year,month) {
    //默认值为当前年月
    if ( !year || !month){
        currentYear = date.getFullYear();
        currentMonth = date.getMonth();
